...
fn random_dynamic_kind<R: Rng + ?Sized>(rng: &mut R) -> DynamicKind {
    match rng.random_range(0..6) {
        ...
        _ => DynamicKind::Suspense {
            mode: random_suspense_mode(rng),
        },
    }
}
    ...
    R: Rng + ?Sized,
{
    match rng.random_range(0..3) {
        ...
        _ => ListEdit::Move {
            from: random_selector(rng),
            to: random_selector(rng),
        },
    }
}
...
fn random_template_edit<R: Rng + ?Sized>(rng: &mut R) -> TemplateEdit {
    match rng.random_range(0..4) {
        0 => TemplateEdit::SetNode {
            node: random_selector(rng),
            kind: random_template_kind(rng),
        },
        1 => TemplateEdit::Roots {
