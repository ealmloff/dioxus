...
fn check_html_renders() -> Element {
    ...
    use_effect(move || {
        ...
        spawn(async move {
            ...
            // If the signal is even, also submit the form. This should reset the page and set the count to 0.
            if signal >= 5 {
                let submit_count = SUBMIT_COUNT.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
                // If we have successfully submitted the form twice, we can close the desktop context.
                if submit_count >= 2 {
