...
fn build_dynamic(spec: &DynamicSpec) -> DynamicNode {
    match spec {
        ...
        DynamicSpec::ComponentA(node) => DynamicNode::Component(VComponent::new(
            GeneratedComponent,
            GeneratedProps { node: node.as_ref().clone() },
            "GeneratedComponent",
        )),
...
impl TemplateCompiler {
    fn compile_node(&mut self, spec: &TemplateNodeSpec, path: &[u8]) -> TemplateNode {
        match spec {
            TemplateNodeSpec::Element { tag, namespace, attrs, children } => {
                let attrs = attrs
                    .iter()
            ...
            TemplateNodeSpec::Text(value) => TemplateNode::Text { text: text_value(*value) },
    ...
    fn compile_attr(&mut self, spec: &TemplateAttrSpec, path: &[u8]) -> TemplateAttribute {
        match spec {
            TemplateAttrSpec::Static { name, value, namespace } => TemplateAttribute::Static {
                name: attr_name(*name),
