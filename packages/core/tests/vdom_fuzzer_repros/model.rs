...
impl Model {
    pub(crate) fn initial() -> Self {
        Self { root: VNodeSpec::minimal(), next_suspense_id: 0 }
    }
    ...
    pub(crate) fn set_kind(&mut self, kind: &TemplateNodeKind) {
        match kind {
            TemplateNodeKind::Element { tag, namespace } => match self {
                Self::Element { tag: current_tag, namespace: current_namespace, .. } => {
                    *current_tag = *tag;
...
pub(crate) enum TemplateAttrSpec {
    Static { name: u8, value: u8, namespace: Option<u8> },
    Dynamic,
}
...
impl SuspenseSpec {
    ...
    pub(crate) fn ready_key(&self) -> SuspenseReadyKey {
        SuspenseReadyKey { id: self.id, generation: self.ready_generation }
    }
